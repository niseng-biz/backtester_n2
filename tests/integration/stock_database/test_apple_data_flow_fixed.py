"""
Apple株価データの取得・格納・読み出しテストスクリプト（修正版）
    ...
    try:
        ...
        try:
            ...
            if company_info_result and company_info_result.get(symbol, False):
                ...
                logger.warning("[WARN] Company info fetch returned False (may be disabled)")
        ...
        try:
            ...
            if financial_data_result and financial_data_result.get(symbol, False):
                ...
                logger.warning("[WARN] Financial data fetch returned False (may be disabled)")
        except Exception as e:
...
def test_data_storage():
    ...
    try:
        ...
        # Connect to database
        db_manager.connect()
        logger.info("[OK] Connected to MongoDB Atlas")
        ...
        if stock_data_result and stock_data_result.get(symbol, False):
            ...
            if stock_data:
                logger.info(f"[OK] Stored {len(stock_data)} stock data records")
            else:
        ...
        try:
            ...
            if company_info_result and company_info_result.get(symbol, False):
                ...
                company_doc = collection.find_one({"symbol": symbol})
                if company_doc:
                    logger.info(f"[OK] Stored company info for {symbol}")
                else:
                    logger.warning("[WARN] No company info found after fetching")
            else:
                logger.warning("[WARN] Company info fetch returned False (may be disabled)")
        except Exception as e:
            logger.error(f"[ERROR] Company info storage error: {e}")
        ...
        try:
            ...
            if financial_data_result and financial_data_result.get(symbol, False):
                ...
                if financial_data:
                    logger.info(f"[OK] Stored {len(financial_data)} financial data records")
                else:
                    logger.warning("[WARN] No financial data found after fetching")
            else:
                logger.warning("[WARN] Financial data fetch returned False (may be disabled)")
        except Exception as e:
            logger.error(f"[ERROR] Financial data storage error: {e}")
...
def test_data_retrieval():
    ...
    try:
        ...
        if stock_data:
            logger.info(f"[OK] Retrieved {len(stock_data)} stock data records")
            ...
            logger.warning("[WARN] No stock data found")
        ...
        if company_info:
            logger.info(f"[OK] Retrieved company info")
        ...
        if financial_data:
            logger.info(f"[OK] Retrieved {len(financial_data)} financial data records")
...
def main():
    """メインテスト関数"""
    logger.info("Apple Stock Data Flow Test (Fixed Version)")
    logger.info("=" * 60)
    ...
    if success_count == total_tests:
        logger.info("[SUCCESS] ALL TESTS PASSED! Apple data flow is working correctly.")
        logger.info("[OK] Yahoo Finance data fetching")
        logger.info("[OK] MongoDB Atlas data storage")
        ...
        if success_count >= 3:
            logger.info("[INFO] Core functionality (stock data) is working correctly.")
            logger.info("[INFO] Company/financial data issues may be due to configuration or API limitations.")
